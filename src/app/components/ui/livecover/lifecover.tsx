...
export default function LifeCover({age}:ClientAgeProps) {
 const [loading,setLoading]= useState<boolean>(false);
  ...
  const calculateByPremium =( rptValue: number, coverAmount:number)=>{
  ...
  const onSubmit = async (data: LifeCoverProps) => {
    ...
    if(lifeDetails){
        setLoading(false);
        console.log(lifeDetails.productLivesAssured);
        const lifeProductsDetails= lifeDetails.productLivesAssured as ProductLivesAssuredProps[];
        const rptValue= lifeProductsDetails?.map(details =>(
          details.RPT
        ));
        const rpt =parseInt(rptValue[0]) ;
        const amount= parseInt(data.amount)
       const results= calculateByPremium(rpt,amount);
       console.log("Calculatons:",results)
    }else{
        setLoading(false);
  ...
  return (
    <div className="p-6">
        ...
        <form className="p-4">
          ...
          <div className="flex lg:flex-row sm:flex-col space-x-4 mt-2">
            <button
